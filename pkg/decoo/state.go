...
type State struct {
	...
	Height int64
	// Index denotes the index of the milestone that is currently being constructed.
	Index uint32
	// PreviousID denotes the ID of the previous milestone.
	PreviousID iotago.MilestoneID
...
func (s *State) MarshalBinary() ([]byte, error) { return json.Marshal(s) }

// UnmarshalBinary must be able to decode the form generated by MarshalBinary.
func (s *State) UnmarshalBinary(data []byte) error { return json.Unmarshal(data, s) }

// Reset resets the complete state after a new milestone has been issued.
func (s *State) Reset(height int64, index uint32, id iotago.MilestoneID) {
	s.Height = height
	s.Index = index
	s.PreviousID = id
	s.Timestamp = 0
	s.ParentByIssuer = map[Key32]iotago.MessageID{}
	s.IssuerCountByParent = map[Key32]int{}
	s.ProofsByMsgID = map[Key32]map[Key32]struct{}{}
	s.Milestone = nil
	s.SignaturesByIssuer = map[Key32]*iotago.Ed25519Signature{}
}

// MilestoneIndex returns the current milestone index of the state.
func (s *State) MilestoneIndex() uint32 {
	s.RLock()
	defer s.RUnlock()
	return s.Index
}
...
func (s *State) Hash() []byte {
	hash := blake2b.Sum256(mustMarshal(s))
	return hash[:]
}

// CheckParent performs syntactic validation on a Proof.
func (s *State) CheckParent(p *tendermint.Parent) uint32 {
	if p == nil {
...
func (s *State) DeliverParent(issuer ed25519.PublicKey, p *tendermint.Parent, _ *Committee) uint32 {
	...
	if _, has := s.ParentByIssuer[issuerKey]; has {
		return CodeTypeReplayError
	}

	// add the parent to the state
	msgID := Key32FromBytes(p.GetMessageId())
	s.ParentByIssuer[issuerKey] = msgID
	s.IssuerCountByParent[msgID]++
	return CodeTypeOK
}

// CheckProof performs syntactic validation on a Proof.
func (s *State) CheckProof(p *tendermint.Proof) uint32 {
	if p == nil {
		return CodeTypeSyntaxError
	}
	// index must not be in the past
...
func (s *State) DeliverProof(issuer ed25519.PublicKey, p *tendermint.Proof, _ *Committee) uint32 {
	// proof must match the current milestone index
	if p.Index != s.Index {
		return CodeTypeStateError
	}
	...
	if s.IssuerCountByParent[Key32FromBytes(msgID)] < 1 {
		return CodeTypeStateError
	}
	// check that the same proof was not issued already
	proofs := s.ProofsByMsgID[Key32FromBytes(msgID)]
	if proofs == nil {
		proofs = map[Key32]struct{}{}
		s.ProofsByMsgID[Key32FromBytes(msgID)] = proofs
	}
...
func (s *State) CheckPartial(p *tendermint.PartialSignature) uint32 {
	if p == nil {
		return CodeTypeSyntaxError
	}
	// index must not be in the past
...
func (s *State) DeliverPartial(issuer ed25519.PublicKey, p *tendermint.PartialSignature, committee *Committee) uint32 {
	...
	if _, has := s.SignaturesByIssuer[Key32FromBytes(issuer)]; has {
		return CodeTypeReplayError
	}
	// the signature must be valid
	essence, err := s.Milestone.Essence()
	if err != nil {
		panic(err)
	}
	...
	s.SignaturesByIssuer[Key32FromBytes(issuer)] = sig
	return CodeTypeOK
}
